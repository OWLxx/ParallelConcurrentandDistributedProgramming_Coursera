...
public final class ParBoruvka extends AbstractBoruvka<ParBoruvka.ParComponent> {
    ...
    public void computeBoruvka(final Queue<ParComponent> nodesLoaded, final SolutionToBoruvka<ParComponent> solution) {
        ...
        ParComponent parComponent = nodesLoaded.poll();
        for (; parComponent != null; parComponent = nodesLoaded.poll()) {
            if (!parComponent.lock.tryLock()) {
                continue;
            }

            if (parComponent.isDead) {
                parComponent.lock.unlock();
                continue;
            }

            Edge<ParComponent> minEdge = parComponent.getMinEdge();
            if (minEdge == null) {
                solution.setSolution(parComponent);
                break;
            }

            ParComponent other = minEdge.getOther(parComponent);

            if (!other.lock.tryLock()) {
                parComponent.lock.unlock();
                nodesLoaded.add(parComponent);
                continue;
            }
            ...
            other.isDead = true;
            parComponent.merge(other, minEdge.weight());
